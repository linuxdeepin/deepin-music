...
class RadioView(TreeView, Logger):
    ...
    def play_item(self, item):
        if item:
            ...
            self.fetch_playlist_and_play(play=True)
            self.save_status()
    ...
    def fetch_playlist_and_play(self, play=False):
    ...
    def get_next_song(self, maunal=False):
        ...
        if self.playlist is None:
            self.fetch_playlist_and_play()
            return

        with self.keep_list_lock():
            self.current_index += 1
            current_index = self.current_index
            self.extend_playlist_and_play()
            return
    ...
    def extend_playlist_and_play(self):
        if len(self.playlist) - self.current_index - 1 < 5:
            songs = fmlib.new_playlist(self.highlight_item.channel_id, self.get_history_sids())
            self.playlist.extend(songs)
            if songs:
                Player.play_new(self.playlist[self.current_index])
