...
QList<MediaMeta> DataBaseService::allMusicInfos()
{
    if (m_MediaMeta.size() > 0) {
        ...
        if (!queryNew.exec()) {
            qCritical() << "1--" << queryNew.lastError();
        }

        while (queryNew.next()) {
            allHash << "2--" << queryNew.value(0).toString();
        }
        ...
        if (! queryNew.exec()) {
            qCritical() << "3--" << queryNew.lastError();
            return m_MediaMeta;
        }
        ...
        if (removeHash.size() > 0) {
            ...
            for (QVariant var : removeHash) {
                ...
                if (!queryNew.exec()) {
                    qCritical() << "4--" << queryNew.lastError();
                }
            }
        }
    }
...
bool DataBaseService::createConnection()
{
    ...
    if (!db.open()) {
        qDebug() << "DataBaseService::createConnection " << db.lastError()
                 << Global::cacheDir()
                 << cachePath;
        return false;
    }
...
int databaseVersionNew()
{
    ...
    query.bindValue(":uuid", DatabaseUUID);
    if (!query.exec()) {
        qWarning() << "---databaseVersionNew " << query.lastError();
        return -1;
    }
