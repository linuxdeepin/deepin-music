...
show_usage() {
    ...
    echo "  $0 --from-step 6   # Start from test report generation"
}
...
step_4_run_tests() {
    ...
    # Ensure we have a report directory
    mkdir -p "$REPORT_DIR"
...
step_5_generate_coverage() {
    ...
    if [ -f "$BUILD_DIR/coverage/filtered.info" ] && [ -s "$BUILD_DIR/coverage/filtered.info" ]; then
        genhtml --output-directory "$BUILD_DIR/coverage/html" --title "Coverage Report" --show-details --legend "$BUILD_DIR/coverage/filtered.info" >> "$REPORT_DIR/coverage_output.log" 2>&1
        if [ $? -eq 0 ]; then
...
step_6_generate_test_report() {
    ...
    if ! command -v python3 &> /dev/null; then
        print_warning "Python 3 not installed, skipping test report"
        return
    fi
...
if [ "$TEST_PASSED" = true ]; then
    ...
    print_error "Unit tests have failures, please check test report"
fi

echo ""
echo "Generated reports:"
echo "  Test report: file://$REPORT_DIR/test_report.html"
echo "  Test data JSON: file://$REPORT_DIR/test_data.json"
if [ "$COVERAGE_SUCCESS" = true ]; then
    echo "  Coverage HTML: file://$BUILD_DIR/coverage/html/index.html"
fi
echo ""
echo "Quick commands:"
echo "  Re-run tests: cd $BUILD_DIR && ctest"
echo "  Re-generate coverage: cd $BUILD_DIR && lcov --directory . --capture"
