...
class TestOutputParser:
    ...
    def parse_gtest_xml_dir(self, results_dir: Path | None = None) -> Dict:
        ...
        root with <testsuite>/<testcase> children, as produced by
        --gtest_output=xml.
        """
        ...
        xml_files = sorted(results_dir.glob("test_*.xml"))
        for xml_file in xml_files:
            try:
                ...
                for suite in root.findall("testsuite"):
                    ...
                    test_info["duration"] += suite_time
                    
                    for case in suite.findall("testcase"):
                        case_name = case.get("name", "")
                        classname = case.get("classname", suite_name)
                        case_time = float(case.get("time", "0"))
                        full_name = f"{classname}.{case_name}"
                        
                        failure_elems = case.findall("failure")
                        if failure_elems:
                            test_info["failed_tests"] += 1
                            test_info["passed"] = False
                            test_info["test_summary"][full_name] = {
                                "status": "FAILED",
                                "duration": case_time
                            }
                            for failure in failure_elems:
                                msg = failure.get("message", "Assertion failed")
                                test_info["detailed_failures"].append({
                                    "suite": classname,
                                    "test_case": case_name,
                                    "full_name": full_name,
                                    "failures": [{
                                        "type": "gtest_xml_failure",
                                        "details": (msg,),
                                        "formatted": msg
                                    }]
                                })
                        ...
                        test_info["total_tests"] += 1
            except Exception as e:
                print(f"Error parsing gtest XML {xml_file}: {e}")
        
        return test_info
    
    def parse_all_results(self, test_passed: bool, test_duration: int) -> Dict:
        """Parse all available test result sources and merge.
        
        Tries ctest test_output.log first; if it yields no tests, falls
        back to gtest XML files in results_dir.
        """
        test_info = self.parse_test_output(test_passed, test_duration)
        
        if test_info["total_tests"] == 0 and self.results_dir:
            xml_info = self.parse_gtest_xml_dir(self.results_dir)
            if xml_info["total_tests"] > 0:
                test_info = xml_info
                test_info["duration"] = int(xml_info["duration"])
